...
function setup() {
	...
	for(var i =  0; i < count; i++) {
		pos.push(new p5.Vector(width/2 + Math.random(-10, 10), 10));
	...
	for(var i =  0; i < count; i++) {
		for (var j = 0; j < count; j++) {
			if (i != j) {
				...
				if (pos[i].dist(pos[j]) < minDistance) {
					var distanceVector = p5.Vector.sub(pos[i], pos[j]);
					var distanceCorrection = (minDistance - pos[i].dist(pos[j])) / 2.0;
					var d = distanceVector.copy();
					var correctionVector = d.normalize().mult(distanceCorrection);

					pos[j].add(correctionVector);
					pos[i].sub(correctionVector)
					...
					var spdFinal = [];
					spdFinal.push(new p5.Vector());
					spdFinal.push(new p5.Vector());

					spdFinal[0].x = spdTemp[0].x + spd[i].x;
					if (spdTemp[0] > 0) {
					...
					spdFinal[1].x = spdTemp[1].x + spd[j].x;
					if (spdTemp[1] > 0) {
					...
					pos[i].add(posFinal[0]);
					pos[j].add(posFinal[1]);
				}
			}
		}
